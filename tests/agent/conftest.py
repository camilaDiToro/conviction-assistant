...
``passages_repo.get_many`` at the module the loop reaches it from
...
patches its own ``passages_repo.get_many``) can still override locally —
...
def _autopatch_passages_repo(
    ...
    """Patch ``passages_repo.get_many`` at the audit module the agent
    loop reaches for it. Returns Passages whose text is a superset of
    every fixture quote. Tests that need a different passage shape can
    re-patch on top.
    ...
    async def fake_get_many(_session: Any, ids: Iterable[str]) -> dict[str, Passage]:
        return {pid: _make_passage(pid) for pid in ids}
