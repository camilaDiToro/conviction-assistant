...
- **Files:** `app/providers/__init__.py` (re-exports), `app/providers/base.py` (protocols + `TokenUsage`), `app/providers/factory.py` (selection by `settings.llm_provider`), `app/providers/openai.py` (`AsyncOpenAI`-backed), `app/providers/stub.py`, `app/providers/_model_prices.json` (vendored from LiteLLM upstream — see `README.md` § "Refreshing model prices"), `app/services/cost.py` (reads the vendored JSON), `scripts/refresh_prices.py`, `tests/providers/test_stub.py`, `tests/providers/test_factory.py`, `tests/providers/test_openai_adapter.py`, `tests/services/test_cost.py`, `tests/fixtures/stub_responses_example.yaml`. **No real OpenAI calls in CI** (the SDK client is mocked with `unittest.mock.patch`).
